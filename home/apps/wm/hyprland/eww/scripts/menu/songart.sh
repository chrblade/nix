...
get_song_art () {
  ...
  if [[ -n "$ART_FROM_BROWSER" ]]; then
    if [[ "$ART_FROM_BROWSER" =~ ^https?:// ]]; then
      ...
      [ $? -ne 0 ] && cp "$HOME/Documents/wallp.jpg" "$TMP_TEMP_PATH"
    else
      # Copy local file or fallback
      cp "$ART_FROM_BROWSER" "$TMP_TEMP_PATH" 2>/dev/null || cp "$HOME/Documents/wallp.jpg" "$TMP_TEMP_PATH"
    fi
  else
    cp "$HOME/Documents/wallp.jpg" "$TMP_TEMP_PATH"
  fi
